    ...
    <div>
      <div className="mb-1.5 flex items-baseline justify-between">
        <label htmlFor={htmlFor} className="text-xs font-semibold uppercase tracking-widest text-white/60">
          {label} {optional && <span className="normal-case text-white/55">{ui.form.optional}</span>}
        </label>
        <AnimatePresence>
          {error && (
            <motion.span
        ...
        <AnimatePresence>
          {showValid && !error && (
            <motion.span
              ...
              className="pointer-events-none absolute end-1 top-1/2 -translate-y-1/2 text-pomelo-blue"
            >
